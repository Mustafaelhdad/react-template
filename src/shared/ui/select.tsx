...
export function Select({ className, children, ...props }: SelectProps) {
  return (
    <select
      className={cn(
        'flex h-10 w-full cursor-pointer appearance-none rounded-md border border-zinc-300 bg-white bg-[length:1.25rem] bg-[right_0.5rem_center] bg-no-repeat px-3 py-2 pr-8 text-sm text-zinc-950 shadow-sm transition-colors focus:border-zinc-950 focus:outline-none disabled:cursor-not-allowed disabled:bg-zinc-100 disabled:opacity-70',
        className,
      )}
      style={{
        backgroundImage:
