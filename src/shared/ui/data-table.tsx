...
export function TableHead({ className, ...props }: ComponentProps<'th'>) {
  return (
    <th
      className={cn(
        'h-10 px-3 text-left align-middle text-xs font-medium uppercase tracking-wide text-zinc-500 [&:has([role=checkbox])]:pr-0',
        className,
      )}
      {...props}
    />
...
export function TableCell({ className, ...props }: ComponentProps<'td'>) {
  return (
    <td
      className={cn(
        'px-3 py-2 align-middle text-zinc-800 [&:has([role=checkbox])]:pr-0',
        className,
      )}
      {...props}
    />
